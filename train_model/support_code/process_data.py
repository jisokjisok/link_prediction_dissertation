...
def Data_Shape(Data):
    ...
    length_B = len(List_B)
    print
    '    数据集长度：' + str(Data.shape[0])
    print
    '    第一列节点长度：(' + str(length_A) + ')'
    print
    '    第二列节点长度：(' + str(length_B) + ')'
    MaxNodeNum = int(max(max(List_A), max(List_B))) + 1
    print
    '    节点数量为：' + str(MaxNodeNum)
    return MaxNodeNum
...
def Divide(NetFile, MatrixAdjacency_Net, MaxNodeNum, NetName):
    ...
    print
    '    训练集邻接矩阵：' + str(MatrixAdjacency_Train.shape)
    print
    '    训练集计划边数：' + str(lengthData - NumTest)
    print
    '    训练集实际边数：' + str((np.nonzero(MatrixAdjacency_Train)[0].shape[0]) / 2)
    print
    '    测试集邻接矩阵：' + str(MatrixAdjacency_Test.shape)
    print
    '    测试集计划边数：' + str(NumTest)
    print
    '    测试集实际边数：' + str((np.nonzero(MatrixAdjacency_Test)[0].shape[0]) / 2)

    writeTrainFile(MatrixAdjacency_Train, NetName)
    writeTestFile(MatrixAdjacency_Test, NetName)
    DivideTime_End = time.clock()
    print
    'DivideTime：%f s' % (DivideTime_End - DivideTime_Start)
